...
export class RegisterComponent {
  ...
  loadCountries() {
    this.authService.getCountries().pipe(takeUntil(this.destroy$)).subscribe({
      ...
      error: () => this.toastService.error('auth.register.toastCountriesFailed'),
    });
  }
  ...
  onSubmit(): void {
    ...
    this.authService.sendOtp({ mobile }).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        ...
        this.toastService.success('auth.register.toastOtpSent');
      ...
      error: (err: any) => {
        ...
        if (err?.status === 409) {
          ...
          this.toastService.error('auth.register.toastOtpSendFailed');
        }
      },
    });
  }
    ...
    this.authService.verifyOtp({ mobile, otp }).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        ...
        if (!res?.success) {
          this.loading.set(false);
          this.otpError.set(true);
          this.toastService.error('auth.register.toastInvalidOtp');
          return;
        }
        this.registerUser();
      },
  ...
  registerUser() {
    ...
    this.authService.register(payload).pipe(takeUntil(this.destroy$)).subscribe({
      ...
      error: (err) => {
        this.loading.set(false);
        this.registeringStage.set('');
        this.toastService.error('auth.register.toastRegistrationFailed');
      },
    });
  }
  ...
  onResendOtp(): void {
    ...
    this.authService.sendOtp({ mobile }).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        this.toastService.success('auth.register.toastOtpResent');
        if (res?.devOtp) this.devOtp.set(res.devOtp);
        this.otpError.set(false);
        this.clearOtpBoxes();
        this.startCountdown();
    ...
    this.authService.googleInitiate({ credential, countryId, allowExistingLogin: false }).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        ...
        if (res.needsUsername) {
          ...
          this.toastService.error('auth.register.toastGoogleAlreadyRegistered');
      ...
      error: (err: any) => {
        ...
        if (!err?.status || err.status >= 500) {
          this.toastService.error('auth.register.toastGoogleUnavailable');
        }
      },
    });
  }
    ...
    this.authService.googleComplete({ credential: this.googleCredential(), username, countryId: this.googleCountryId() ?? undefined })
      ...
      .subscribe({
        ...
        error: (err: any) => {
          ...
          if (!err?.status || err.status >= 500) {
            this.toastService.error('auth.register.toastGoogleSomethingWrong');
          }
        },
      });
  }
